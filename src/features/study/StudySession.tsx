...
export function StudySession({ cards, setName, nextSetName, onNextSet, onExit }: { cards: Card[]; setName: string; nextSetName?: string; onNextSet?: () => void; onExit: () => void }) {
  ...
  if (phase === "result") { const seconds = Math.max(1, Math.round((finishedAt - startedAt) / 1000)); const initialCorrect = cards.length - initialIncorrect; return <main className="page result-page"><section className="result-card"><p className="eyebrow">SESSION COMPLETE</p><h1>おつかれさまでした</h1><p className="completion">すべての単語を正答しました。</p><div className="score"><strong>{Math.round((initialCorrect / cards.length) * 100)}<small>%</small></strong><span>初回正答率</span></div><dl><div><dt>初回誤答</dt><dd>{initialIncorrect}語</dd></div><div><dt>復習ラウンド</dt><dd>{round - 1}回</dd></div><div><dt>判定回数</dt><dd>{attempts}回</dd></div><div><dt>学習時間</dt><dd>{Math.floor(seconds / 60)}分{seconds % 60}秒</dd></div></dl>{onNextSet && nextSetName && <button className="primary" onClick={onNextSet}>次のセットへ <span>→</span><small>{nextSetName}</small></button>}<button className={onNextSet ? "secondary full-button" : "primary"} onClick={start}>もう一度学習する</button><button className="text-button" onClick={onExit}>セット詳細へ戻る</button></section></main>; }
