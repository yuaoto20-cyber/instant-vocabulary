...
export function StudySession({ cards, setName, onExit }: { cards: Card[]; setName: string; onExit: () => void }) {
  ...
  const speak = useCallback((word: string) => { if (!audio || !("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(word); utterance.lang = "en-US"; utterance.rate = 0.9; window.speechSynthesis.speak(utterance); }, [audio]);
  useEffect(() => { if (phase !== "study" || paused || !current) return; speak(current.english); const timer = window.setTimeout(() => setRevealed(true), delay); return () => window.clearTimeout(timer); }, [phase, paused, current, delay, speak]);
  useEffect(() => { const pauseForVisibility = () => { if (document.hidden && phase === "study") { window.speechSynthesis?.cancel(); setPaused(true); } }; document.addEventListener("visibilitychange", pauseForVisibility); return () => document.removeEventListener("visibilitychange", pauseForVisibility); }, [phase]);

  const start = () => { const first = random ? shuffle(cards) : [...cards]; setQueue(first); setRound(1); setRoundSize(first.length); setPosition(0); setRevealed(false); setPaused(false); setInitialIncorrect(0); setAttempts(0); setSwipeX(0); setStartedAt(Date.now()); setFinishedAt(0); setPhase("study"); };
  const judge = useCallback((result: Result) => {
    if (!revealed || !current) return;
    window.speechSynthesis?.cancel(); setRevealed(false); setSwipeX(0); setAttempts((count) => count + 1); if (result === "incorrect" && round === 1) setInitialIncorrect((count) => count + 1);
  ...
  if (phase === "setup") return <main className="page setup-page"><section className="intro"><p className="eyebrow">STUDY SET</p><h1>{setName}</h1><p>{cards.length}語を、一定のリズムで学習します。</p></section><section className="setup-card"><fieldset><legend>自動反転までの時間</legend><div className="delay-options">{[500, 1000, 1500, 2000, 2500, 3000, 4000, 5000].map((ms) => <button key={ms} className={delay === ms ? "selected" : ""} onClick={() => setDelay(ms)}>{(ms / 1000).toFixed(1)}秒</button>)}</div></fieldset><label className="switch-row"><span><strong>出題順</strong><small>{random ? "ランダム" : "登録順"}</small></span><input type="checkbox" checked={random} onChange={(event) => setRandom(event.target.checked)} /></label><label className="switch-row"><span><strong>英語音声</strong><small>{audio ? "自動再生する" : "自動再生しない"}</small></span><input type="checkbox" checked={audio} onChange={(event) => setAudio(event.target.checked)} /></label><button className="primary start" onClick={start}>学習を開始 <span>→</span></button><button className="text-button" onClick={onExit}>セット詳細へ戻る</button></section></main>;
  ...
  return <main className="page study-page" tabIndex={0} onKeyDown={onKeyDown}><header><div><span>{round === 1 ? "通常ラウンド" : `誤答復習 · Round ${round}`}</span><strong>{position + 1} <small>/ {roundSize}</small></strong></div><button className="pause-button" onClick={() => setPaused(true)} aria-label="一時停止">Ⅱ</button></header><div className="progress"><i style={{ width: `${progress}%` }} /></div><section className="card-area"><div className={`vocab-card ${revealed ? "is-revealed" : ""} ${swipeX ? "is-dragging" : ""}`} style={{ "--swipe-x": `${swipeX}px`, "--swipe-r": `${swipeX / 30}deg` } as CSSProperties} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}><div className="card-face card-front"><p>ENGLISH</p><h1>{current?.english}</h1><button onClick={() => current && speak(current.english)} aria-label="音声を再生">◖</button><span>意味を思い出してください</span></div><div className="card-face card-back"><p>JAPANESE</p><h1>{current?.japanese}</h1><span>左右にスワイプして判定</span></div></div></section><section className="actions"><button className="incorrect" disabled={!revealed} onClick={() => judge("incorrect")}><kbd>←</kbd><span>誤答</span></button><button className="correct" disabled={!revealed} onClick={() => judge("correct")}><span>正答</span><kbd>→</kbd></button></section><p className="keyboard-hint">キーボードの ← → でも判定できます</p>{paused && <div className="pause-overlay" role="dialog" aria-modal="true"><div><p className="eyebrow">PAUSED</p><h2>学習を一時停止しました</h2><p>再開すると、現在の英単語からもう一度表示します。</p><button className="primary" onClick={() => { setRevealed(false); setSwipeX(0); setPaused(false); }}>再開する</button><button className="text-button" onClick={onExit}>学習を終了</button></div></div>}</main>;
}
